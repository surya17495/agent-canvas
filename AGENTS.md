...
- **Test specs**:
  ...
  - `mock-llm-diff-stress.spec.ts` — Diff view responsiveness stress test. Creates 12 files of varying sizes (1-line text up to 500-line Python modules) in a single conversation, then measures render times: diff list appearance after refresh, expanding small/medium/large diffs, and rapid sequential expand/collapse cycles. Asserts all timings stay below thresholds (30s list render, 10-20s expand depending on size, 5s per rapid cycle). File content is generated programmatically via `generatePythonContent()`. The trajectory is built dynamically from a `FILE_SPECS` manifest. Allows 1 missing file due to potential agent-server internal LLM call padding.
