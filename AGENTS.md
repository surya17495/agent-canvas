...
- Dev-mode request fanout note: HAR/CSV captures count each Vite-served ESM module as a separate request, so slow local loads are dominated by eager import breadth (`/src/**/*.ts(x)` + `/node_modules/.vite/*`) rather than production bundling. High-impact reductions came from (1) replacing scattered `lucide-react`/`react-icons` imports with the shared local `src/components/shared/icons.tsx` entrypoint and SVG assets, (2) lazy-loading home/sidebar overlays that are closed on first paint (`settings-modal`, conversation panel, user menu/add-backend modal, workspace browser/manage modal, repo/workspace launcher variants), and (3) removing dead route-level eager imports like the stray `PrefetchPageLinks` in `src/routes/home.tsx`.
