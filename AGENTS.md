...
- Auth modes for `agent-canvas` (dev and production):
  ...
  - **Gate-7 decision (Centri milestone U1, private beta):** For the current private beta, public mode ships with a **user-pasted session key** (the flow described above), not an injected/secret-managed key. This is the accepted beta posture. Two guarantees back it:
    - *Never silently invents or exposes a key:* `makeDefaultLocalBackend()` returns `null` when no key is baked/injected, so the registry is not seeded with a fabricated key; `scripts/static-server.mjs` enforces that `--session-api-key` and `--auth-required` are mutually exclusive; `ApiKeyEntryScreen` always renders the key field empty. Regression coverage: `__tests__/api/backend-registry/default-backend.test.ts`.
    - *Paste flow usability:* `ApiKeyEntryScreen` pre-fills the backend **name** (the existing backend's name on key rotation, else `DEFAULT_LOCAL_BACKEND_NAME`) so a beta user only pastes the key and clicks Connect; the name field stays editable. Unit coverage: `__tests__/components/backends/api-key-entry-screen.test.tsx`.
    - *State clarity:* loading → `AgentServerBootstrapLoading`; empty/unauthorized → `ApiKeyEntryScreen`; backend-unavailable → `MissingAgentServerScreen` (all gated in `src/root.tsx`).
    - **Pre-launch requirement (not yet built):** before general (non-beta) launch, public mode must support a **private injected / secret-managed session key path** (e.g. key delivered via a secrets manager / server-side injection rather than pasted by the user), so operators are not required to hand raw keys to end users. Tracked as a launch gate, intentionally out of scope for the beta.
...
- **Centri state root (fork change)**: ALL default runtime state now lives under the Centri-owned root `~/.centri/canvas` (from `config/defaults.json` → `paths.stateRoot`), NOT upstream's `~/.openhands`. Rationale: on the U1 dev VM the deployed fork and a vanilla OpenHands install collided in `~/.openhands` (shared settings/secrets, shared `secret-key.txt`, interleaved conversations) — the state-dir collision tracked in the centri repo (SPEC §10). `scripts/state-paths.mjs` is the single fork-owned module defining the root; `dev-safe.mjs`, `logger.mjs`, `dev-with-automation.mjs` import their defaults from it, and `docker/entrypoint.sh` derives the same root from `CONFIG_STATE_ROOT` in `defaults.env`. The directory SHAPE below the root is identical to upstream's shape below `~/.openhands` (`agent-canvas/` state subdir, `automation/automations.db` beside it, `storage/`, `workspaces/`), so every `dirname(stateDir)`-based derivation is unchanged. Env overrides (`OH_CANVAS_SAFE_STATE_DIR`, `OH_PERSISTENCE_DIR`, key-path vars) keep working and keep precedence. There is NO auto-migration from `~/.openhands/agent-canvas` — launchers print a one-time hint instead (moving data out of a directory that may belong to a live vanilla OpenHands install is a human decision). NOTE: older learnings in this file that mention `~/.openhands/agent-canvas/…` paths predate this change — read them as `~/.centri/canvas/agent-canvas/…` (the exception is the agent-server's own `~/.openhands/skills/` — that belongs to the agent server, not to canvas state, and is untouched). When syncing from upstream, keep the state-paths import sites and never let a merge reintroduce `~/.openhands` defaults.
