...
- Mini setup server (`scripts/setup-server.mjs`): a ~110 line HTTP server on `127.0.0.1:18099` that bridges the browser frontend and Docker lifecycle. Endpoints: `GET /setup/status` (Docker availability), `POST /setup/docker` (start container), `DELETE /setup/docker` (stop container). Started by `dev-with-automation.mjs` and routed through the ingress at `/setup/*`. In standalone Vite dev mode (`dev:mock` / `dev:frontend`), `vite.config.ts` proxies `/setup` to `http://127.0.0.1:${SETUP_SERVER_PORT || 18099}`. The frontend client is `src/api/setup-service.ts`. Reuses Docker image/mount logic from `dev-docker.mjs` (container name: `agent-canvas-dev-docker-backend`, port 18002). In a future Electron app, this role moves to the main process via IPC.

- Choose Backend step (`src/components/features/onboarding/steps/choose-backend-step.tsx`): replaces `CheckBackendStep` in onboarding Step 1. Multi-select: Local (always available) + Docker (opt-in, spawns via setup server). Uses `getSetupStatus()` and `startDockerBackend()` from `setup-service.ts`. The old `CheckBackendStep` (`check-backend-step.tsx`) is still in the codebase but no longer wired into the onboarding modal.
