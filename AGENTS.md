...
- `/server_info` tool capability metadata from `software-agent-sdk` PR #3028 ended up shipping as `usable_tools` (not `available_tools`). Frontend compatibility code should prefer `usable_tools`, fall back to `available_tools`, and default to allowing tools when neither field is present so older servers still work.
