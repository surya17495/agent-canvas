...
**Stale state from earlier serial tests** — Mock-LLM tests run serially (`workers: 1`) against a real agent-server. Earlier tests (conversation, automation) persist settings on the server. If your test needs "first-run" state (e.g. onboarding), use `interceptSettingsForFirstRun(page)` from `mock-llm-helpers.ts` in the `beforeGoto` callback — it intercepts `GET /api/settings` to strip stale `agent_settings.llm` values so the frontend sees schema defaults. This is necessary because (a) the agent-server PATCH API rejects null/empty values for validated fields like `model` and `api_key` (422), and (b) the `inferInitialView` function checks ALL schema fields against their defaults to decide between Basic/Advanced view. The interception must be registered AFTER `routeSessionApiKey` (Playwright LIFO order).
